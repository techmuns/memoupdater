...
const SECTION_BLOCKS: Record<CanonicalSectionId, string> = {
  ...
  sec_thesis_scorecard: [
    ...
    "  When the current market price isn't verified from a primary source, write 'current price not verified from a primary source' rather than inventing a number. Sanity-check any price against the 52-week range where sourced and flag a price that sits outside it.",
  ...
  sec_corporate_events: [
    "This is the 'Corporate Events (Last 12 Months)' section. It must cover TWO things: (1) the material corporate events of the last ~12 months, and (2) governance / accounting-quality flags a diligent PM must not miss.",
    "Required content:",
    ...
    "- `bullets`: up to 4 MAX. Lead with the most thesis-relevant events, then the most serious governance/accounting flags. Each bullet: '<What happened> — <Why it matters in 1 phrase>' (≤ 240 chars), ending with a directional tag: [improves] / [weakens] / [mixed] / [neutral].",
    "  Event scope: M&A (state deal SIZE and HOW funded — debt vs equity/QIP — and any guarantees/contingent liabilities), divestments, restructuring, capex, major contract wins/losses, fund-raises (QIP/NCD/rights — note dilution), refinancing, buybacks, dividends, strategic pivots, litigation, regulatory action.",
    "  Governance/accounting flags to scan for and surface when sourced: KMP churn (CFO/CEO/auditor resignations or repeated changes), auditor change or qualifications, shift from net-cash to net-debt or rising leverage, growing reliance on 'adjusted' vs reported metrics (and the SIZE of the gap), working-capital / receivable-days deterioration, promoter pledge changes, related-party transactions, contingent liabilities/guarantees.",
    "  Name any governance/accounting concern plainly and tag it [weakens] or [mixed] — do NOT bury it. If none surfaced, do not invent one.",
    "- `signal`: `positive` if the event mix improves the case; `negative` if events or governance flags weaken it; `watch` if mixed; `neutral` only when no findings.",
    "- `bridge`: omit.",
  ].join("\n"),
