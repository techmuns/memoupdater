...
const SHARED_SYSTEM_LINES = [
  ...
  "- ZERO MARGIN OF ERROR (HARD): every number in your output MUST come from (a) the server-fetched live-quote/fundamentals block, (b) an explicit number in the original memo / its DNA / its understanding digest, OR (c) a number quoted in a research finding. Do NOT compute, estimate, interpolate, round-for-cleanliness, or paraphrase a number. If a metric isn't available from those three sources, the cell MUST read 'not surfaced' (in a bridge) or be omitted (in prose) — NEVER fabricated.",
...
const SECTION_BLOCKS: Record<CanonicalSectionId, string> = {
  ...
  sup_valuation_detail: [
    ...
    "",
    "Required content:",
    "- `bridge`: REQUIRED — include EVERY row below for which the live-quote / fundamentals block provides the LATEST value AND a corresponding memo-date value is sourced (from the memo DNA, the memo understanding digest, or a research finding). Use `original` = value AT the memo date, `latest` = value NOW, `readThrough` = re-rate / de-rate amount with one short driver phrase. Rows:",
    "  1. Stock price (memo vs now). Read-through: absolute return + annualised (CAGR) + relative-to-index where sourced.",
    ...
    "  8. 52-week range (omit `original`; `latest` = 'Rs LOW – Rs HIGH').",
    "  9. Implied price using the ORIGINAL multiple on LATEST trailing EPS, vs the actual market price.",
    "  10. Peer trailing P/E gap (vs 1–2 key peers) when sourced.",
    ...
    "  CURRENT VALUES — SOURCE OF TRUTH: the live-quote / fundamentals block in the user prompt (price, mkt cap, EV, trailing P/E, trailing EV/EBITDA, P/B, trailing EPS, 52w range) is AUTHORITATIVE. Use it VERBATIM. Do NOT search for an alternative; do NOT compute one from other numbers.",
    ...
    "  EMPTY-CELL RULE (HARD — zero margin of error): a cell with no sourced value MUST read exactly 'not surfaced' (for the latest) or 'not in memo' (for the original). NEVER estimate, NEVER interpolate, NEVER write '~' or 'approx' to make a number look plausible.",
  ...
  sup_eps_bridge: [
    ...
    "  2. Latest REPORTED trailing EPS (sourced from the live-quote / fundamentals block or a research finding).",
    ...
    "  EMPTY-CELL RULE (HARD): if a memo-anchor EPS doesn't exist, `original` reads 'not in memo'; if a latest figure isn't sourced, `latest` reads 'not surfaced'. NEVER estimate.",
  ...
  sup_financials_actuals: [
    ...
    "  - `latest` cell: the latest REPORTED actual (no estimates, no forecasts, no consensus). If not sourced, MUST read 'not surfaced'.",
    ...
    "  EMPTY-CELL RULE (HARD): never fabricate, estimate, interpolate, or 'approximately' a value. Use 'not in memo' / 'not surfaced' instead.",
