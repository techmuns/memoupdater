...
const SHARED_SYSTEM_LINES = [
  ...
  "",
  "LANGUAGE STYLE — plain English, no jargon (HARD):",
  "- Prefer one short word over one long word. Prefer one sentence over two. Cut adjectives that don't carry a fact.",
  "- BANNED filler words and jargon-y phrases: 'leverages', 'unlocks', 'optimises', 'best-in-class', 'compounder', 'platformisation', 'flywheel', 'narrative', 'in-line with expectations', 'in-line with thesis', 'broadly intact', 'cleanly delivered', 'thesis-altering', 'high-conviction', 'monetisation rails', 'go-forward', 'ahead of plan'. If a thought needs one of these, rewrite it with concrete facts.",
  "- Allowed finance shorthand (use only when it appears in the source / industry usage): P/E, EV/EBITDA, EBITDA, PAT, EPS, ROE, ROCE, FY, QoQ, YoY, capex, dividend, M&A. Spell out anything more obscure on first use.",
  "- Avoid hedging strings like 'it may be that', 'arguably', 'could potentially'. Either say it (with the source) or omit it.",
  "- Investor-direct voice: name the company, name the number, name the date. No 'the company' when you can say the ticker / short name.",
  "",
...
const SECTION_BLOCKS: Record<CanonicalSectionId, string> = {
  ...
  sec_thesis_scorecard: [
    ...
    "- `body`: 1–2 sentences max. Cover (a) RETURN ATTRIBUTION computed from the numbers — if EPS grew X% and the stock returned Y%, the multiple moved roughly (Y − X)%; a de-rating means the return came from earnings DESPITE multiple contraction — say it exactly that way. (b) Skip any 'intact / broken / on track' verdict; the per-pillar bullets below carry the verdict pillar-by-pillar.",
  ...
  sup_valuation_detail: [
    ...
    "",
    "Required content:",
    "- `bridge`: REQUIRED. Include EVERY row below; only drop a row if BOTH `original` and `latest` would be empty. Use `original` = value AT the memo date, `latest` = value NOW, `readThrough` = re-rate / de-rate amount with one short driver phrase. Rows:",
    "  1. Stock price (memo vs now). Read-through MUST match the scorecard's return numbers EXACTLY — same absolute %, same CAGR, same period — never re-compute from a different anchor. If the scorecard said '+26%, ~12% p.a.', this row says the same.",
