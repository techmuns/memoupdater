...
const PASS_BLOCKS: Record<ResearchPassId, string> = {
  ...
  valuation_market: [
    ...
    "AUTHORITATIVE LIVE DATA: if '# 1. Company identity' carries a 'Current price (server-fetched, live)' line — and/or a 'Fundamentals (server-fetched, live, do NOT re-verify)' line — those numbers are AUTHORITATIVE for this run. Do NOT spend a web_search call on current price, trailing EPS, trailing P/E, trailing EV/EBITDA, market cap, enterprise value, P/B or 52-week range; use them verbatim and spend the saved budget on peer multiples and segment-level reads. ONLY when those lines are absent should you do a live-quote web_search of the form '<companyName> share price' / '<ticker> stock price today'; acceptable live-quote sources are Google Finance, Yahoo Finance, NSE/BSE/SEC/SEBI quote pages, Bloomberg, Reuters, Tickertape, Screener.in. Capture price together with its as-of date.",
    ...
    "Report a SINGLE representative CURRENT trailing P/E and SINGLE current trailing EV/EBITDA — NEVER a wide nonsensical range (e.g. '46x–186x'). If sources disagree, use the most recent value and state the basis (consolidated; reported vs adjusted EPS).",
    "NO FORWARD METRICS: do NOT spend search budget on forward P/E / forward EV/EBITDA / forward EPS / FY+1 multiples — the memo does not use them. Trailing/current only.",
    "Capture, where sourced, the trailing inputs the valuation bridge needs: current price (with date), market cap, enterprise value, trailing P/E, trailing EV/EBITDA, P/B, trailing EPS, 52-WEEK RANGE; the absolute return since the memo date with its ANNUALISED (CAGR) figure and the relevant INDEX return over the same window; and 1–2 key PEER trailing multiples for the peer gap.",
