...
Because the SDK is loaded via a minified script, the global `window.MunshotDashboardSDK` is a namespace, not the client itself. You MUST initialize it using the factory method before accessing any subscription or request methods:

```typescript
// 1. Get the global namespace
const sdkLib = (window as any).MunshotDashboardSDK;

// 2. Extract the correct factory function
const factory = sdkLib?.createDashboardClientSdk ?? sdkLib?.createClient;

// 3. Initialize the client
const client = factory({
  dashboardId: "my-dashboard",
  dashboardName: "My Dashboard Title"
});
```
...
Every generated dashboard MUST implement the `dashboard.capture.snapshot` request handler:

```typescript
client.onRequest("dashboard.capture.snapshot", () => {
  return {
    // 1. Current applied filters/context
    context: {
      ticker: currentTicker,
      dateRange: currentDateRange,
    },
    // 2. Currently selected UI elements (if any)
    selection: activeSelection,
    // 3. The raw data currently rendered in the widgets
    data: {
      kpis: currentKpiData,
      primaryChart: currentChartData,
